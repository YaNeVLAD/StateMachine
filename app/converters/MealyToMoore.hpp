...
CollectUniqueStateOutputPairs(const mealy_state::transitions_t& mealy_transitions)
{
	...
	moore_state& mooreState)
{
	for (const auto& [mealy_state_name, mealy_output] : pairs)
	{
		...
		mooreState.state_ids.insert(new_moore_name);
		mooreState.outputs[new_moore_name] = mealy_output;
	}
}

inline std::string CreateMooreStartState(const std::string& mealy_start_state, moore_state& mooreState)
{
	std::string moore_start_name = mealy_start_state + "_start";
	mooreState.state_ids.insert(moore_start_name);
	mooreState.outputs[moore_start_name] = "INITIAL";

	return moore_start_name;
}

inline moore_state::transitions_t CreateMooreTransitions(
	const mealy_state& mealy_state,
	const std::set<std::pair<std::string, std::string>>& pairs,
	const std::string& moore_start_name)
{
	moore_state::transitions_t moore_transitions;

	for (const auto& [from, to] : mealy_state.transitions)
	{
		if (const auto& [from_state, input] = from; from_state == mealy_state.initial_state_id)
		{
...
using MealyToMooreConverter = fsm::converter<fsm::mealy_machine, fsm::moore_machine>;

template <>
struct fsm::converter<fsm::mealy_machine, fsm::moore_machine>
{
	[[nodiscard]] moore_machine operator()(const mealy_machine& mealy) const
	{
		moore_state mooreState;
		...
		mooreState.initial_state_id = mealy_state.initial_state_id;

		mooreState.transitions = ::details::CreateMooreTransitions(
			mealy_state, unique_pairs, mooreState.initial_state_id);

		mooreState.current_state_id = mooreState.initial_state_id;

		return moore_machine(mooreState);
	}
};
